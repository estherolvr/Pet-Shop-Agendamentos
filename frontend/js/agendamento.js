...
// SVG placeholder em base64 (substitui o placeholder.com)
            ...
            tr.innerHTML = `
                <td>
                    <div class="agendamento-pet">
                        ...
                        <div>
                            <div class="nome-pet">${item.nome_pet}</div>
                            <div class="raca-pet">${item.raca_pet}</div>
                        </div>
                    </div>
                </td>
...
async function salvarAgendamento(event) {
    ...
    try {
        ...
        if (!validarData(formAgendamento.data.value)) {
            throw new Error('Por favor, insira uma data válida');
        }
        if (!validarHorario(formAgendamento.horario.value)) {
            throw new Error('Por favor, insira um horário válido');
        }

        const formData = new FormData(formAgendamento);
        
        // Adiciona foto atual se estiver em edição e não foi trocada
        const fotoAtual = document.getElementById('fotoAtual');
        if (fotoAtual && !formAgendamento.foto_pet.files.length) {
            formData.append('foto_atual', fotoAtual.value);
        }

        const token = localStorage.getItem('token');
        const url = idEditando 
            ? `${API_BASE_URL}/api/agendamentos/${idEditando}`
            : `${API_BASE_URL}/api/agendamentos`;

        // CORREÇÃO: Removido o header 'Content-Type' para permitir que o navegador
        // defina o boundary automaticamente para FormData
        ...
        mostrarNotificacao(
            idEditando ? 'Agendamento atualizado com sucesso!' : 'Agendamento realizado com sucesso!',
            'sucesso'
        );
        ...
        // Mantém o estado de edição se estiver editando
        if (idEditando) {
            document.getElementById('btn-submit').textContent = 'Atualizar Agendamento';
        }
    } finally {
        ...
        submitBtn.textContent = idEditando ? 'Atualizar Agendamento' : 'Agendar';
